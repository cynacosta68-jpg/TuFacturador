...
function secret() {
  const s = process.env.ARCANUM_SESSION_SECRET || config.masterKey || config.apiKey;
  if (!s) {
    throw new Error(
      '[arcanum] No hay secreto de sesion. Defini ARCANUM_SESSION_SECRET en el .env ' +
      '(la app no opera con un secreto por defecto).'
    );
  }
  return s;
}
