...
function resolveKey() {
  ...
  // Sin master key explicita:
  // En PRODUCCION debe fallar: nunca operar con una clave autogenerada que se
  // pueda perder (dejaria los certificados imposibles de descifrar).
  if (config.isProd) {
    throw new Error(
      '[arcanum] ARCANUM_MASTER_KEY es OBLIGATORIA en produccion. Definila por entorno ' +
      '(openssl rand -hex 32) y respaldala: sin ella no se pueden descifrar los certificados.'
    );
  }
  // En desarrollo/homologacion: generamos y persistimos una en disco para no
  // perder los datos entre reinicios (con aviso).
