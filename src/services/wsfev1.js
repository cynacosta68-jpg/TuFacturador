...
async function auth(authCuit, certCuit) {
  // Se autentica con el certificado de certCuit (la representante) pero declara
  // authCuit en el bloque Auth (el representado). Si no hay representante, son iguales.
  const ta = await getAccessTicket(normalizeCuit(certCuit || authCuit), SERVICE);
  return (
    '<ar:Auth>' +
    `<ar:Token>${ta.token}</ar:Token>` +
    `<ar:Sign>${ta.sign}</ar:Sign>` +
    `<ar:Cuit>${normalizeCuit(authCuit)}</ar:Cuit>` +
    '</ar:Auth>'
  );
}
...
async function lastAuthorized(cuit, ptoVta, tipoCbte, certCuit) {
  const cc = normalizeCuit(certCuit || cuit);
  return withTokenRetry(cc, SERVICE, config.env, async () => {
    const inner =
      (await auth(cuit, cc)) +
...
async function authorizeInvoice(cuit, inv) {
  ...
  const certCuit = inv.representante ? normalizeCuit(inv.representante) : c;
  return withTokenRetry(certCuit, SERVICE, config.env, () => emitir(c, inv, { ptoVta, tipoCbte, concepto, idem }));
}

async function emitir(cuit, inv, { ptoVta, tipoCbte, concepto, idem }) {
  const certCuit = inv.representante ? normalizeCuit(inv.representante) : normalizeCuit(cuit);
  let numero = inv.numero;
  if (!numero) {
    const last = await lastAuthorized(cuit, ptoVta, tipoCbte, certCuit);
  ...
  const detalle =
    ...
    (inv.condicionIvaReceptor != null ? `<ar:CondicionIVAReceptorId>${parseInt(inv.condicionIvaReceptor, 10)}</ar:CondicionIVAReceptorId>` : '') +
  ...
  const inner = (await auth(cuit, certCuit)) + `<ar:FeCAEReq>${cabecera}${detalle}</ar:FeCAEReq>`;
      ...
      await db.query(
        `INSERT INTO comprobantes
           (cuit, entorno, punto_venta, tipo_cbte, numero, cae, cae_vto, resultado, fecha,
            importe_total, doc_tipo, doc_nro, idempotency_key, raw, meta)
